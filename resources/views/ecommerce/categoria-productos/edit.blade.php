        ...
        <div class="row">
            <div class="col-lg-12 white-bg">
                <div class="wrapper wrapper-content">
                    ...
                    <form role="form" name="FormRegistro" id="FormRegistro" action="{{route('categoriaProductos.update',$categoriaProducto)}}" method="POST" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="form-group">
                            <label for="Nombre">{{ trans('labels.nombre') }}</label>
                            <input type="text" class="form-control" id="Nombre" name="Nombre" value="{{old('Nombre', $categoriaProducto->nombre )}}" maxlength="50" autocomplete="off" placeholder="" required>
                            @error('Nombre')
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" class="form-control" id="Resumen" name="Resumen" value="{{old('Resumen', $categoriaProducto->resumen)}}" maxlength="200" autocomplete="off" placeholder="" >
                            @error('Resumen')
                            <span class="small text-danger">{{$message}}</span>
                            @enderror
                        </div>
